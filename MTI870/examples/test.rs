...
use render::{
    ...
    shapes::{json_to_shape, triangle::Triangle},
    ...
    if json.contains_key("material") {
        ...
        for (name, dir) in &directions {
            ...
            let lambda_pdf = |wi: &Vec3| -> f64 {
                material.pdf(dir, wi)
                // votrecodeici!("Decommetez le code si dessus");
                // 1.0
            };
            let lambda_sample = |sample: &Vec2| -> Vec3 {
                if let Some(res) = material.sample(dir, sample) {
                    res.wi
                } else {
                    Vec3::zero()
                }
        ...
        // // DEVOIR 3
        // unimplemented!("Devoir 3: Echantillonnage de formes");
        ...
        let shape = if shape_refs.len() == 1 {
            match shape_refs.pop().unwrap() {
                ...
                render::shapes::JsonShape::Mesh(maybe_mesh) => {
                    let mesh = maybe_mesh.expect("Mesh loading failed in test harness");
                    let mesh = Arc::new(mesh);
                    let mut root = render::shapes::shape_group::SimpleShapeGroup::default();
                    for face_id in 0..mesh.face_positions_idx.len() {
            ...
            for shape_ref in shape_refs {
                match shape_ref {
                    ...
                    render::shapes::JsonShape::Mesh(maybe_mesh) => {
                        if let Some(mesh_raw) = maybe_mesh {
                            ...
                            error!("Skipping mesh that failed to load in test harness");
                        }
                    }
                }
            }
