...
impl BVHNode {
    ...
    pub fn new_leaf(
        ...
        Self {
            aabb,
            info: BVHNodeInfo::Leaf {
                frist_primitive: frist_primitive,
    ...
    pub fn frist_primitive(&self) -> usize {
        match self.info {
            ...
            _ => unimplemented!(), // Not possible, have you already converted the node?
        }
    }
    ...
    pub fn to_node(&mut self, left: usize) {
...
impl BVH {
    pub fn from_json(json: &HashMap<String, JsonValue>) -> Self {
        ...
        let axis_selection = match &axis_selection_str[..] {
            ...
            _ => {
                warn!(
                    "Unknown axis_selection: {}, use Round robin instead.",
                    axis_selection_str
                );
        ...
        let builder = match &builder_str[..] {
            ...
            _ => {
                warn!("Unknown builder: {}, use median instead.", builder_str);
                Builder::EMedian
            }
        };
        ...
        */
        ...
        match node.info {
            ...
            BVHNodeInfo::Node { left } => {
                ...
                let d1 = n1.aabb.hit(r).unwrap_or(std::f64::MAX);
                let d2 = n2.aabb.hit(r).unwrap_or(std::f64::MAX);
        ...
        let offset = match self.builder {
            ...
            Builder::ESAH => {
                if false {
                    ...
                    // 16 bins
                    let mut scores = vec![0.0; 16 - 1];
                    for o in 0..3 {
                        let delta = (aabb_centroid.max[o] - aabb_centroid.min[o]) / 16.0;

                        // Compute bins
                        let mut bins_count = vec![0; 16];
                        ...
                        for id in 0..15 {
                            ...
                            scores[id_left - 1] = tmp.area() * nb as f64;
                        }
                        tmp = AABB::default();
                        nb = 0;
                        for id in 0..15 {
