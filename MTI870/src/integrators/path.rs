...
use crate::{
    ...
    shapes::Shape,
    vec::{luminance, Color3, Frame, Vec3},
};
        ...
        while depth < self.max_depth {
            ...
            match selection.into_nearest() {
                Some(SurfaceHit::Analytic(its)) => {
                    ...
                    let le = material.emission(&wo);
                    accumulated_radiance = accumulated_radiance + throughput.mul_element_wise(le);

                    if let Some(res) = material.sample(&wo, &sampler.next2d()) {
