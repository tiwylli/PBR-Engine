...
fn pixel_to_direction(p: Vec2, image_size: Vec2u) -> Vec3 {
    spherical_coordinates_to_direction(
        p.x * 2.0 * std::f64::consts::PI / image_size.x as f64,
        p.y * std::f64::consts::PI / image_size.y as f64,
    )
}
...
fn direction_to_pixel(d: Vec3, image_size: Vec2u) -> Vec2u {
    ...
    Vec2u::new(
        (sc.0 * image_size.x as f64 / (2.0 * std::f64::consts::PI)) as u32,
        (sc.1 * image_size.y as f64 / std::f64::consts::PI) as u32,
    )
}
...
pub fn generate_histogram<'a>(
    ...
    let progress = ProgressBar::new(image_size.y as u64);
    progress.set_style(
    ...
    for y in 0..image_size.y {
        ...
        for x in 0..image_size.x {
            ...
            for _ in 0..nb_samples {
                let pos_img = Vec2::new(x as f64, y as f64) + sampler.next2d();
                let dir = pixel_to_direction(pos_img, image_size);
                let sin_theta = (1.0 - dir.z * dir.z).max(0.0).sqrt();
                let pixel_area = (std::f64::consts::PI / image_size.y as f64)
                    * (std::f64::consts::PI * 2.0 / image_size.x as f64)
                    * sin_theta;
                let pdf_val = pdf_f(&dir);
                if pdf_val.is_infinite() || pdf_val.is_nan() {
                    if !nan_or_inf {
                        warn!("PDF is NaN or Inf at ({}, {}) -- dir: {:?}", x, y, dir);
                    }
                    nan_or_inf = true;
                    continue;
                }
            ...
            *pdf.at_mut(x, y) = acc / nb_samples as f64;
        }
    }
    // Normalize integral
    integral /= nb_samples as f64;
    progress.is_finished();

    // Compute sample histogram
    let mut histogram = Array2d::with_size(image_size.x, image_size.y, 0.0);
    let normalisation =
        1.0 / (std::f64::consts::PI * (2.0 * std::f64::consts::PI) * nb_samples as f64);
    progress.reset();
    ...
    info!(
        "Integrale de la pdf (devrais etre proche de 1): {}",
        integral
    );
    info!("99.95% percentile de la pdf: {}", exposure);
    info!(
        "La difference entre l'histogramme et la pdf (devrais etre proche de 0): {}",
        difference / (image_size.x * image_size.y) as f64
    );
