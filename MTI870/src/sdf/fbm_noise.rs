...
fn hash_vec3(i: Vec3) -> f64 {
    let mut n = i.x as i64 * 15731 + i.y as i64 * 789221 + i.z as i64 * 1376312589;
    n = (n << 13) ^ n;
    let nn = n * (n * n * 15731 + 789221) + 1376312589;
...
fn sd_base_lattice(p: Vec3) -> f64 {
    ...
    for dx in 0..=1 {
        for dy in 0..=1 {
            for dz in 0..=1 {
                let corner = Vec3::new(dx as f64, dy as f64, dz as f64);
...
fn sd_base_simplex(p: Vec3) -> f64 {
    ...
    let sum = p.x + p.y + p.z;
    let i = Vec3::new((p.x + sum * K1).floor(), (p.y + sum * K1).floor(), (p.z + sum * K1).floor());
    let d0 = p - (i - Vec3::new((i.x + i.y + i.z) * K2, (i.x + i.y + i.z) * K2, (i.x + i.y + i.z) * K2));
    ...
    let i2 = Vec3::new(1.0 - e.x * (1.0 - e.z), 1.0 - e.y * (1.0 - e.x), 1.0 - e.z * (1.0 - e.y));
    ...
    let sph = |d: Vec3, r: f64| d.magnitude() - r * r * 0.55;

    sph(d0, r0).min(sph(d1, r1)).min(sph(d2, r2).min(sph(d3, r3)))
}
...
fn json_to_warp(json: &HashMap<String, JsonValue>) -> Matrix3<f64> {
    if let Some(matrix) = json.get("warp_matrix") {
        let rows: &Vec<JsonValue> = matrix
            .get()
            .expect("warp_matrix must be an array of 3 rows");
        assert!(
            rows.len() == 3,
            "warp_matrix must contain exactly 3 rows"
        );
        ...
        Matrix3::new(
            0.0, 0.80, 0.60, -0.80, 0.36, -0.48, -0.60, -0.48, 0.64,
        )
    }
}
...
impl SdfFbmNoise {
    pub fn from_json(
        ...
        let noise_variant = json
            ...
            .unwrap_or(NoiseVariant::Lattice);
        ...
        let material = json.get("material").map(|value| {
            ...
            Arc::clone(
                materials
                    .get(name)
                    .unwrap_or_else(|| panic!("Unknown material `{}` for fbm noise", name)),
            )
        });
