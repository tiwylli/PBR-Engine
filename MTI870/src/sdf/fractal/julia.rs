...
/// Placeholder implementation for a Julia-set style fractal.
pub struct SdfJulia {
...
impl SdfJulia {
    pub fn from_json(
        ...
        let material = json.get("material").map(|value| {
            let name: &str = value
                .get::<String>()
                .expect("Julia `material` must be a string");
            Arc::clone(
                materials
                    .get(name)
                    .unwrap_or_else(|| panic!("Unknown material `{}` for Julia", name)),
            )
        });

        let settings = parse_object_settings(json);
        let constant = json_to_vec3(json, "constant", Vec3::new(0.355, 0.355, 0.355));
        let max_iterations = json_to_f64(json, "max_iterations", 25.0) as u32;
        let bailout = json_to_f64(json, "bailout", 6.0);
