...
use cgmath::{ElementWise, InnerSpace, SquareMatrix, Transform, Zero};
...
use render::{
    ...
    vec::{Color3, Mat3, Mat4, Point3, Vec3, Vec4},
};
...
fn task_1_maths() {
    ...
    */
    ...
    let degree = cgmath::dot(v0.normalize(), v1.normalize())
        .acos()
        .to_degrees();
    ...
    */
    ...
    let mut rangen = ChaCha8Rng::seed_from_u64(0);
    ...
    let point = Point3::new(rangen.random(), rangen.random(), rangen.random());
    let vector = Vec3::new(rangen.random(), rangen.random(), rangen.random()).normalize();

    // appliquez la matrice "m2" pour transformer le point et le vecteur
    ...
    let new_point = Point3::from_homogeneous(m2 * point.to_homogeneous());
    let m2_3x3 = Mat3::new(
        m2.x.x, m2.x.y, m2.x.z, m2.y.x, m2.y.y, m2.y.z, m2.z.x, m2.z.y, m2.z.z,
    );
    let new_vector = m2_3x3 * vector;
...
fn task_2_images(path: &str) {
    ...
    */
    ...
    // Faite la couleur "purple" (violet) en additionant le rouge et bleu
    let purple = red + blue;
    info!("purple: {:?}", purple);
    ...
    */
    ...
    /* Changer l'image nore en effectuant une interpolation bilineaire
    ...
    */
    for x in 0..im_black.size_x() {
        for y in 0..im_black.size_y() {
            // Changer la ligne ci-dessous et calculez l'interpolation bilineaire
            ...
            let tx = x as f64 / im_black.size_x() as f64;
            let ty = y as f64 / im_black.size_y() as f64;
            *im_black.at_mut(x, y) = ty * (tx * white + (1.0 - tx) * green)
                + (1.0 - ty) * (tx * blue + (1.0 - tx) * red);
        }
    }
    ...
    // Calculer de la moyenne des pixels composont l'image
    let avg = {
    ...
    // Transformer l'image en noir et blanc et stocker la dans im_lum
    // Vous pouvez utiliser la fonction "double luminace(const Color3& c)" définie dans vec.h
    for x in 0..im.size_x() {
        for y in 0..im.size_y() {
            let lum = cgmath::dot(*im.at(x, y), avg);
            // let lum = cgmath::dot(*im.at(x, y), Vec3::new(0.2126, 0.7152, 0.0722));
            let pixel = im.at_mut(x, y);
            pixel.x = lum;
            pixel.y = lum;
            pixel.z = lum;
        }
    }
    image_save("luminance.png", &im).unwrap();
}
