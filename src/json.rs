        ...
        match json_val {
            JsonValue::Array(json) => {
                if json.len() == 3 {
                    ...
                    return Err(crate::Error::WrongDimensionJson("vec3", json.clone(), 3));
                }
            }
            _ => {
...
use crate::{
    deg2rad,
    medium::{HenyeyGreenstein, HomogeneousMedium, Medium, PhaseFunction},
...
impl TryFrom<JsonValue> for JsonVec3 {
    ...
    fn try_from(value: JsonValue) -> Result<Self, Self::Error> {
        match value {
            JsonValue::Number(v) => Ok(Self(Vec3::new(v, v, v))),
            JsonValue::Array(v) => {
                if v.len() == 3 {
                    let v: Vec<f64> = v[0..3].iter().map(|v| *v.get().unwrap()).collect();
                    Ok(Self(Vec3::new(v[0], v[1], v[2])))
                } else {
...
impl TryFrom<JsonValue> for JsonVec2 {
    ...
    fn try_from(value: JsonValue) -> Result<Self, Self::Error> {
        match value {
            JsonValue::Number(v) => Ok(Self(Vec2::new(v, v))),
            JsonValue::Array(v) => {
                if v.len() == 2 {
                    let v: Vec<f64> = v[0..2].iter().map(|v| *v.get().unwrap()).collect();
                    Ok(Self(Vec2::new(v[0], v[1])))
                } else {
...
pub fn json_to_vec3(json: &HashMap<String, JsonValue>, name: &str, default: Vec3) -> Vec3 {
    ...
    TryInto::<JsonVec3>::try_into(json_val).map_or(default, |v| v.0)
}
...
pub fn json_to_vec2i(json: &HashMap<String, JsonValue>, name: &str, default: Vec2i) -> Vec2i {
    ...
    let json_val = json[name].clone();
    TryInto::<JsonVec2i>::try_into(json_val).map_or(default, |v| v.0)
}
...
pub fn json_to_vec2(json: &HashMap<String, JsonValue>, name: &str, default: Vec2) -> Vec2 {
    ...
    let json_val = json[name].clone();
    TryInto::<JsonVec2>::try_into(json_val).map_or(default, |v| v.0)
}

#[must_use]
pub fn json_to_phase_function(json: &HashMap<String, JsonValue>) -> PhaseFunction {
    let phase_type = json_to_string(json, "type", "isotropic");
    match phase_type.as_str() {
        "isotropic" => PhaseFunction::Isotropic,
        "henyey-greenstein" => {
            let g = json_to_f64(json, "g", 0.0);
            PhaseFunction::HenyeyGreenstein(HenyeyGreenstein { g })
        }
        _ => panic!("Unknown phase function type: {phase_type}"),
    }
}

pub fn json_to_medium(json: &HashMap<String, JsonValue>) -> crate::Result<Box<dyn Medium>> {
    let medium_type = json_to_string(json, "type", "homogeneous");
    match medium_type.as_str() {
        "homogeneous" => {
            let sigma_a = json_to_vec3(json, "sigma_a", Vec3::new(0.1, 0.1, 0.1));
            let sigma_s = json_to_vec3(json, "sigma_s", Vec3::new(0.1, 0.1, 0.1));
            let density = json_to_f64(json, "density", 1.0);

            let phase = json
                .get("phase")
                .map_or(PhaseFunction::Isotropic, |phase_json| {
                    json_to_phase_function(phase_json.get().unwrap())
                });
        ...
        _ => Err(crate::Error::UncoveredCase("medium", json.clone())),
    }
}
...
#[cfg(test)]
mod tests {
    use cgmath::{SquareMatrix, assert_abs_diff_eq};
    use tinyjson::JsonValue;
