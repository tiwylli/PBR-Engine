...
impl SDFDirectIntegrator {
    ...
    fn explicit_emitter(
        ...
        (fbsdf_cos / ps.pdf).mul_element_wise(le)
    }
}
    ...
    fn li(&self, ray: &Ray, scene: &Scene, sampler: &mut dyn Sampler) -> Color3 {
        ...
        match self.direct_type {
            ...
            DirectType::Mis => {
                ...
                if !surface.material().have_delta() {
                    contrib += direct_emitter_mis_surface(
                        &surface,
                        scene,
                        sampler,
                        &frame,
                ...
                if let Some(bsdf_sample) = surface.material().sample(
                    &wo,
                    &surface.uv,
                    &surface.position,
                    &sampler.next2d(),
                    ...
                    if let Some(light_hit) = hit {
                        if surface_hit_has_emission(&light_hit) {
                            ...
                            let pdf_bsdf = surface.material().pdf(
                                &wo,
                                &bsdf_sample.wi,
                                &surface.uv,
                                &surface.position,
                            ...
                            contrib += bsdf_sample.weight.mul_element_wise(le) * mis_w;
                        }
                    } else {
                        contrib += bsdf_sample
                            .weight
