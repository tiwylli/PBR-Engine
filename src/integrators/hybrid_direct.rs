...
use crate::{
    ...
    vec::{Color3, Frame, Vec3, luminance},
};
...
impl HybridDirectIntegrator {
    ...
    fn russian_roulette(weight: &mut Color3, sampler: &mut dyn Sampler) -> bool {
    ...
    fn explicit_emitter(
        ...
        let max_t = crate::constants::RAY_EPS.mul_add(-2.0, dist);
        ...
        let fbsdf_cos = surface
            .material()
            .evaluate(wo, &wi_local, &surface.uv, &surface.position);
        let mut weight = fbsdf_cos / ps.pdf;
        if !Self::russian_roulette(&mut weight, sampler) {
            return Color3::from_value(0.0);
        }
...
impl SamplerIntegrator for HybridDirectIntegrator {
    ...
    fn li(&self, ray: &Ray, scene: &Scene, sampler: &mut dyn Sampler) -> Color3 {
        ...
        match self.direct_type {
            DirectType::Bsdf => self.sample_bsdf(&surface, scene, sampler, &frame, &wo),
            DirectType::Naive => self.hemisphere_naive(&surface, scene, sampler, &frame, &wo),
            DirectType::Emitter => {
                if surface.material().have_delta() {
