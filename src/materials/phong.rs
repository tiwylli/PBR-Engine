use super::{Material, SampledDirection};
use crate::{
    samplers::{pdf_cosine_hemisphere, sample_cosine_hemisphere, sample_cosine_hemisphere_power},
    texture::{Texture, json_to_texture},
    vec::{Color3, Frame, Point3, Vec2, Vec3, luminance},
};

use cgmath::InnerSpace;
use std::collections::HashMap;
use std::f64::consts::PI;
use tinyjson::JsonValue;

//ChadGPT idea to make an enum for blinn
// Chadgpt a aidé au refactoring pour utiliser l'enum intelligemment
#[derive(Copy, Clone)]
enum SpecModel {
    Phong,
    Blinn,
}

/// Matériau Phong/Blinn-Phong : mixture d'un lobe diffus et d'un lobe spéculaire
pub struct Phong {
    kd: Texture<Color3>, // Couleur diffuse
    ks: Texture<Color3>, // Couleur spéculaire
    exponent: f64,       // Exposant (s)
    model: SpecModel,    // Phong ou Blinn-Phong
    normal_map: Option<Texture<Vec3>>,
}

impl Phong {
    /// Construction à partir d'un JSON
    #[must_use]
    pub fn from_json(json: &HashMap<String, JsonValue>) -> Self {
        let kd = json_to_texture(json, "kd", Color3::new(0.2, 0.2, 0.2));
        let ks = json_to_texture(json, "ks", Color3::new(0.8, 0.8, 0.8));

        let mut exponent = if let Some(JsonValue::Number(n)) = json.get("exponent") {
            *n
        } else {
            30.0
        };
        if exponent < 1.0 {
            exponent = 1.0;
        }

        // Flag JSON: "model": "phong" | "blinn"  (défaut: phong)
        let model = if let Some(JsonValue::String(s)) = json.get("model") {
            if s.eq_ignore_ascii_case("blinn") || s.eq_ignore_ascii_case("blinn-phong") {
                SpecModel::Blinn
            } else {
                SpecModel::Phong
            }
        } else {
            SpecModel::Phong
        };

        let normal_map = super::json_to_normal_map(json);

        Self {
            kd,
            ks,
            exponent,
            model,
            normal_map,
        }
    }

    #[allow(clippy::similar_names)]
    fn get_prob_spec(&self, uv: &Vec2, p: &Point3) -> f64 {
        // p(spec) = lum(ks) / (lum(ks) + lum(kd))
        let lks = luminance(&self.ks.get(uv, p));
        let lkd = luminance(&self.kd.get(uv, p));
        let denom = lks + lkd;
        if denom > 0.0 {
            (lks / denom).clamp(0.0, 1.0)
        } else {
            0.0
        }
    }

    fn eval_spec(&self, wo: &Vec3, wi: &Vec3, uv: &Vec2, p: &Point3) -> Color3 {
        match self.model {
            SpecModel::Phong => {
                // r = direction de réflexion de -wo autour de n=(0,0,1)
                let r = Vec3::new(-wo.x, -wo.y, wo.z).normalize();
                let ndotr = r.dot(*wi).max(0.0);
                // Normalisation Phong: (s+2)/(2π) * (r·wi)^s
                self.ks.get(uv, p)
                    * ((self.exponent + 2.0) / (2.0 * PI))
                    * ndotr.powf(self.exponent)
            }
            SpecModel::Blinn => {
                // h = normalize(wo + wi)
                let sum = *wo + *wi;
                if sum.magnitude2() == 0.0 {
                    return Color3::new(0.0, 0.0, 0.0);
                }
                let h = sum.normalize();
                let ndoth = h.z.max(0.0); // n=(0,0,1)
                // Normalisation Blinn-Phong classique: (s+8)/(8π) * (n·h)^s
                self.ks.get(uv, p)
                    * ((self.exponent + 8.0) / (8.0 * PI))
                    * ndoth.powf(self.exponent)
            }
        }
    }

    fn pdf_spec(&self, wo: &Vec3, wi: &Vec3) -> f64 {
        match self.model {
            SpecModel::Phong => {
                let r = Vec3::new(-wo.x, -wo.y, wo.z).normalize();
                let ndotr = r.dot(*wi).max(0.0);
                // PDF de l'échantillonnage Phong autour de r
                (self.exponent + 1.0) * ndotr.powf(self.exponent) / (2.0 * PI)
            }
            SpecModel::Blinn => {
                // On a échantillonné le half-vector h autour de n via power-cosine
                // p(h) = (s+1)/(2π) * (n·h)^s
                // p(wi) = p(h) / (4 * |wo·h|)
                let sum = *wo + *wi;
                if sum.magnitude2() == 0.0 {
                    return 0.0;
                }
                let h = sum.normalize();
                let ndoth = h.z.max(0.0);
                let wodoth = wo.dot(h).abs().max(1e-12);
                let pdf_h = (self.exponent + 1.0) * ndoth.powf(self.exponent) / (2.0 * PI);
                pdf_h / (4.0 * wodoth)
            }
        }
    }
}

impl Material for Phong {
    fn sample(&self, wo: &Vec3, uv: &Vec2, p: &Point3, sample: &Vec2) -> Option<SampledDirection> {
        if wo.z <= 0.0 {
            return None; // below the surface
        }

        // Choix hiérarchique du lobe
        let mut xi = *sample;
        let wi: Vec3;

        let prob_spec = self.get_prob_spec(uv, p);
        if prob_spec > 0.0 && xi.x < prob_spec {
            // Lobe spéculaire
            xi.x /= prob_spec;

            match self.model {
                SpecModel::Phong => {
                    // Échantillonne autour de r avec power-cosine
                    let s_local = sample_cosine_hemisphere_power(&xi, self.exponent);
                    let r = Vec3::new(-wo.x, -wo.y, wo.z).normalize();
                    let frame = Frame::new(&r);
                    let cand = frame.to_world(&s_local);
                    if cand.z <= 0.0 {
                        return None;
                    }
                    wi = cand;
                }
                SpecModel::Blinn => {
                    // Échantillonne le half-vector h autour de n (Z+)
                    let h = sample_cosine_hemisphere_power(&xi, self.exponent);
                    // Réflexion de wo autour de h: wi = 2(wo·h)h - wo
                    let wodoth = wo.dot(h);
                    let cand = (h * (2.0 * wodoth)) - *wo;
                    if cand.z <= 0.0 {
                        return None;
                    }
                    wi = cand.normalize();
                }
            }
        } else {
            // Lobe diffus (cosine hemisphere)
            let denom = (1.0 - prob_spec).max(1e-12);
            xi.x = (xi.x - prob_spec).max(0.0) / denom;
            wi = sample_cosine_hemisphere(&xi);
        }

        // Mixture PDF et BSDF*cos via les méthodes evaluate/pdf
        // chadgpt added this when i was looking for problems of phong
        // Adrien m'a ensuite dit de l'ajouter dans blend et fresnel blend
        let pdf = self.pdf(wo, &wi, uv, p);
        if pdf <= 0.0 {
            return None;
        }
        let fcos = self.evaluate(wo, &wi, uv, p);
        Some(SampledDirection {
            wi,
            weight: fcos / pdf,
        })
    }

    fn evaluate(&self, wo: &Vec3, wi: &Vec3, uv: &Vec2, p: &Point3) -> Color3 {
        if wi.z <= 0.0 {
            return Color3::new(0.0, 0.0, 0.0);
        }
        let cos_theta = wi.z.max(0.0);

        // Diffus Lambert: kd/π
        let eval_diffuse = self.kd.get(uv, p) / PI;

        // Spéculaire selon le modèle
        let eval_spec = self.eval_spec(wo, wi, uv, p);

        // BSDF * cos(theta)
        (eval_diffuse + eval_spec) * cos_theta
    }

    fn pdf(&self, wo: &Vec3, wi: &Vec3, uv: &Vec2, p: &Point3) -> f64 {
        if wi.z <= 0.0 {
            return 0.0;
        }
        let pdf_spec = self.pdf_spec(wo, wi);
        let pdf_diff = pdf_cosine_hemisphere(wi);
        // self.prob_spec * pdf_spec + (1.0 - self.prob_spec) * pdf_diff
        let prob_spec = self.get_prob_spec(uv, p);
        prob_spec.mul_add(pdf_spec, (1.0 - prob_spec) * pdf_diff)
    }

    fn have_delta(&self) -> bool {
        // Exposant fini -> non-delta
        false
    }

    fn emission(&self, _wo: &Vec3, _uv: &Vec2, _p: &Point3) -> Color3 {
        Color3::new(0.0, 0.0, 0.0)
    }

    fn have_emission(&self) -> bool {
        false
    }

    fn get_normal_map_value(&self, uv: &Vec2, p: &Point3) -> Vec3 {
        super::get_normal_map_value_helper(self.normal_map.as_ref(), uv, p)
    }

    fn have_normal_map(&self) -> bool {
        self.normal_map.is_some()
    }
}
