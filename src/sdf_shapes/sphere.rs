...
impl SDFObject for SdfSphere {
    ...
    fn gradient(&self, world_p: Point3) -> Option<Vec3> {
        let local = self.world_to_object.point(&world_p);
        let local_v = Vec3::new(local.x, local.y, local.z);
        if local_v.magnitude2() <= f64::EPSILON {
            return None;
        }
        let local_normal = local_v.normalize();
        let world_normal = self.object_to_world.normal(&local_normal);
