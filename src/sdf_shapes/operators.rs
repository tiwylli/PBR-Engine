...
use crate::{
    aabb::{AABB, intersect_aabb, merge_aabb},
    materials::Material,
...
use super::{
    ...
    sdf_object::{SDFObject, json_to_sdf_object, parse_object_settings},
};
...
fn load_children(
    ...
    let list = json
        .get(key)
        .unwrap_or_else(|| panic!("SDF operator `{key}` field missing"));
...
fn shrink_axis(min: &mut f64, max: &mut f64, rmin: f64, rmax: f64) {
    if rmax <= *min || rmin >= *max {
        return;
    }
    if rmin <= *min && rmax < *max {
        *min = rmax.min(*max);
    } else if rmax >= *max && rmin > *min {
        *max = rmin.max(*min);
    }
}

fn subtract_bounds(left: &AABB, right: &AABB) -> AABB {
...
impl SdfUnion {
    ...
    pub fn from_json(
        ...
        assert!(
            !children.is_empty(),
            "SDF union requires at least one child"
        );
        ...
        let material = json.get("material").map(|value| {
            ...
            Arc::clone(
                materials
                    .get(name)
                    .unwrap_or_else(|| panic!("Unknown material `{name}` for SDF union")),
            )
        });
...
impl SdfIntersection {
    ...
    pub fn from_json(
        ...
        assert!(
            !children.is_empty(),
            "SDF intersection requires at least one child"
        );

        let mut bounds = children[0].world_bounds();
        for child in children.iter().skip(1) {
            if let Some(intersection) = intersect_aabb(&bounds, &child.world_bounds()) {
                bounds = intersection;
            } else {
                bounds = AABB::default();
                break;
        ...
        let material = json.get("material").map(|value| {
            ...
            Arc::clone(
                materials
                    .get(name)
                    .unwrap_or_else(|| panic!("Unknown material `{name}` for SDF intersection")),
            )
        });
...
impl SdfDifference {
    ...
    pub fn from_json(
        ...
        assert!(
            parts.len() == 2,
            "SDF difference expects exactly two children"
        );

        let left_bounds = parts[0].world_bounds();
        let right_bounds = parts[1].world_bounds();
        let bounds = subtract_bounds(&left_bounds, &right_bounds);
        ...
        let material = json.get("material").map(|value| {
            ...
            Arc::clone(
                materials
                    .get(name)
                    .unwrap_or_else(|| panic!("Unknown material `{name}` for SDF difference")),
            )
        });
