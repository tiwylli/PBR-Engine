...
impl RaymarchSettings {
    ...
    #[must_use]
    pub fn with_overrides(&self, json: &HashMap<String, JsonValue>) -> Self {
        let mut merged = *self;
...
impl RaymarchResult {
    ...
    #[must_use]
    pub const fn miss(status: RaymarchStatus) -> Self {
        Self { status, hit: None }
    }

    /// Helper for successful hits.
    #[must_use]
    pub const fn success(hit: RaymarchHit) -> Self {
        Self {
...
pub fn raymarch(ray: &Ray, sdf_obj: &dyn SDFObject, settings: RaymarchSettings) -> RaymarchResult {
    ...
    let Some((mut entry_t, mut exit_t)) = ray_bounds_interval(ray, &bounds) else {
    ...
    let object_step_scale = sdf_obj.step_scale().clamp(1.0e-3, 1.0);
    let step_multiplier = settings.step_clamp * object_step_scale;
    let mut t = entry_t;
    let mut steps = 0_u32;
    ...
    while steps < settings.max_steps && t <= exit_t {
        ...
        let mut step = distance * step_multiplier;
        if step <= 0.0 {
            step = settings.hit_epsilon;
        }

        let min_advance: f64 = 1.0e-7;
...
pub fn compute_normal(world_p: Point3, sdf_obj: &dyn SDFObject, eps: f64) -> Vec3 {
    if let Some(gradient) = sdf_obj.gradient(world_p)
        && gradient.magnitude2() > f64::EPSILON
    {
    ...
    if gradient.magnitude2() <= f64::EPSILON {
        Vec3::unit_y()
    } else {
        gradient.normalize()
    }
