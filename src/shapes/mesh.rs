...
impl Mesh {
    ...
    pub fn from_json(
        ...
        assert!(
            json.contains_key("filename"),
            "Mesh need to contains filename"
        );
        ...
        if res.is_err() {
            error!("Failed to load file: {}", filename.display());
            return None;
        }
        ...
        for m in models {
            ...
            if !mesh.texcoords.is_empty() {
                ...
                for uv in mesh.texcoords[..].chunks_exact(2) {
                    uvs.push(Vec2::new(f64::from(uv[0]), f64::from(uv[1])));
                }
            }
        }
        ...
        if !uvs.is_empty() && !normals.is_empty() {
